...
from api.models.responses import Prompt
...
@app.get("/prompt")
def read_item(prompt: Union[str, None] = None) -> Prompt:
    res=send_prompt(prompt)
    # send_to_phone(res)
    return res


def message_category(message: Union[str, None] = None,phone_num: Union[str, None] = None):
    ...
    send_to_phone(respone_msg,phone_num)
    return respone_msg

# Define a route to handle incoming requests
@app.post('/whatsapp')
async def chat(phone_num:Union[str, None] = Form(None),body: str = Form(...)):
   response = MessagingResponse() 
   print(phone_num)
#    message_category(Body)

   msg = response.message(message_category(body,phone_num))
